...
export function renderHarnessState(state: HarnessState, limits: HarnessLimits): string {
  if (state.entries.length === 0) return `${state.scope} harness revision ${state.revision}: empty`
  ...
  if (newest.length < state.entries.length) lines.push(`- [${state.entries.length - newest.length} older entries omitted]`)
  return lines.join('\n')
}
