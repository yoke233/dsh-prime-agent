 ...
 */
function primeDelivery(parent: Agent | undefined): 'quiet' | 'wakeup' {
 ...
 */
export function installPrimeReport(childCtx: Context, ctx: Context): () => void {
  ...
  try {
    ...
    const diagnosed = new Error(
      'dsh-prime-agent/subagent-report: failed to register the `report` tool for a continuable child. '
      + 'If the cause is a duplicate tool name, the host tool-subagent-report row was not disabled — '
      + "re-sync the report pair in dsh-prime-agent's cordis.patch.yml against the upstream base bundle.",
      { cause: error },
    )
    try {
      disposeSection()
      ...
      throw new AggregateError(
        [diagnosed, rollbackError],
        'failed to register the prime report tool and roll back its prompt guidance',
      )
    }
    throw diagnosed
  }
