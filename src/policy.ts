...
- Use edit for one exact in-place replacement, apply_patch for a cohesive strict Add/Update patch whose every target can be preflighted, and write only when intentionally creating or replacing a complete file. A multi-file apply_patch is ordered rather than transactional: if a mutation fails, inspect its partial-application error before compensating.
