...
function parseFileHeader(line: string, lineNumber: number): { operation: 'add' | 'delete' | 'update'; path: string } {
  if (line.startsWith(ADD_FILE)) return { operation: 'add', path: line.slice(ADD_FILE.length) }
  if (line.startsWith(DELETE_FILE)) return { operation: 'delete', path: line.slice(DELETE_FILE.length) }
  if (line.startsWith(UPDATE_FILE)) return { operation: 'update', path: line.slice(UPDATE_FILE.length) }
  throw invalidHunk(
    `Expected '${ADD_FILE}{path}', '${DELETE_FILE}{path}', or '${UPDATE_FILE}{path}'.`,
    lineNumber,
  )
}
...
function invalidHunk(message: string, line: number, path?: string): PatchError {
