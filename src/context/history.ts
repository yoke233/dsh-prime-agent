import { deriveEventMessage, isAppendSurfaceEvent, SessionSeq, type Session } from '@deepseek-ai/dsh-session'
import type { ContentBlock } from '@deepseek-ai/dsh-llm'
import type {} from '@deepseek-ai/dsh-tools'

/** Project recorded content without exposing provider replay state or private reasoning. */
function visibleContent(blocks: readonly ContentBlock[]): unknown[] {
  return blocks.flatMap((block): unknown[] => {
    if (block.type === 'reasoning') return []
    if (block.type === 'tool-result') return [{ ...block, content: visibleContent(block.content) }]
    return [block]
  })
}
...
function record(seq: number, kind: string, body: unknown) {
...
export function searchHistory(session: Session, query = '', before = session.seq as number, limit = 10) {
  if (!Number.isSafeInteger(before) || before < 0 || before > session.seq) throw new Error('before must be a history offset in this session')
  ...
  const hits: { seq: number; kind: string; preview: string }[] = []
...
export function readHistory(session: Session, seq: number, offset = 0, limit = 8000) {
  if (!Number.isSafeInteger(seq) || seq < 0 || seq >= session.seq) throw new Error('seq must identify an existing event in this session')
  const record = historyRecord(session, seq)
  if (record === undefined) throw new Error('this event is not readable conversation history')
  if (!Number.isSafeInteger(offset) || offset < 0 || offset > record.text.length) throw new Error('offset is outside this record')
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > 8000) throw new Error('limit must be between 1 and 8000')
...
export function historyDirectory(session: Session): string {
  const entries: { seq: number; excerpt: string }[] = []
