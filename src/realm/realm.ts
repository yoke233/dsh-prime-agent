...
export function emptyRealmMetrics(): RealmMetrics {
  return {
    completionsFull: 0,
 ...
 */
function parseRunMetrics(raw: unknown): RealmRunMetrics {
  ...
  for (const key of ['retained', 'rejected'] as const) {
...
function parseWorkerMessage(raw: unknown): RealmToHost | undefined {
  ...
  switch (message.type) {
    ...
    case 'done': {
      ...
      if (message.json === undefined) return { type: 'done', runId, nonce, metrics }
...
export class PersistentRealm {
  ...
  private settle(entry: RunEntry, result: CodeRunResult): void {
    ...
    if (result.error?.kind === 'output-limit') this.counters.outputLimits += 1
