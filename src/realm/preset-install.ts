...
class MissingPresetSourceError extends Error {
  constructor(sourceDir: string, options?: ErrorOptions) {
    super(
      `dsh-prime-agent: packaged preset directory ${JSON.stringify(sourceDir)} is missing or is not a directory; `
      + 'the published package must ship agent-presets/prime',
      options,
    )
  }
}
 ...
 */
export async function installPrimePreset(options: PresetInstallOptions): Promise<PresetInstallResult> {
  ...
  if (await occupied(targetDir)) return 'already-present'
  ...
  if (!sourceIsDirectory) throw new MissingPresetSourceError(sourceDir)
  ...
  try {
    ...
    if (process.platform !== 'win32') await tightenModes(staging)
