...
export class HistoryWindowEngine extends BasicCompactionEngine {
  protected override async summarize(input: { readonly messages: readonly Message[] }, agent: Agent, signal?: AbortSignal) {
    ...
    return { summary: [{ type: 'text' as const, text }], provider: 'dsh-prime-agent', model: 'history-directory' }
  }
}

function owner(exec: ToolRunContext): Agent {
  exec.signal.throwIfAborted()
  if (exec.agent === undefined || exec.agent.session === undefined || exec.agent.id !== exec.agent.session.id) throw new Error('context tools require an owning Agent and Session')
  return exec.agent
}
...
export async function apply(ctx: Context, config: Config): Promise<void> {
  ...
  if (!(engine instanceof HistoryWindowEngine)) throw new Error('Prime context window service did not mount in its isolated scope')
  ...
  ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
    ...
    if (generation !== undefined) {
      ...
      if (agent.session.surface.replaceGeneration === generation) {
        const engine = ctx.get('compaction')
        if (engine === undefined) throw new Error('context window service is unavailable')
        const result = await engine.compactIfNeeded(agent, 'context-overflow', signal).catch(error => {
          if (error instanceof NoUsefulWindow) return null
          throw error
        })
        if (result === null) agent.session.append('user/message', createUserMessage({
