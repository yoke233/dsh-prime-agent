...
> 实现更新：2026-09-07，第 11 节补充批量子调用失败收尾、证据指导与任务评测；当前契约以 `docs/architecture.md` 为准，真实模型效果仍待评测。
...
## 11. 2026-09-07：失败收尾、证据保真与可运行评测

本轮以第 10 节已实现的历史窗口为基线，没有新增变量目录、动态 SDK、自动摘要或费用预算。

- **子调用失败收尾**：复现 `queryMany` 已拒绝后其他工作分支仍领取后续请求，且这些成功结果无法返回的问题。保留整批失败契约，改为首个失败停止领取、取消同批在途请求并等待收尾，再返回失败下标；父取消与批间隔离分别验证。单批并发限制仍可能被并行多批叠加，不宣称它是 Session 费用上限。
- **证据指导**：移除 cell 与输出成本的绝对排序、单调用原样返回的机械红旗。示例保留来源与省略数量；归约保留关键例外和未决项。变量是否保留取决于后续需要，是否复用取决于来源是否仍有效。
- **恢复指导**：重要进展和用户纠正时更新已有 task notes。补充确定性回放，覆盖笔记为空或过时、至少两次自动窗口替换、纠正消息已不在最近八条目录中，仍能回取原始记录并只提交一次正确结果。它验证接口可用性，不证明模型会自行选择这条轨迹。
- **任务级评测**：新增独立 opt-in 模型套件，覆盖来源变化、长材料否定条件、query 失败／截断和窗口恢复。使用业务提交值、证据 ID 和提交次数评分；调用用量按公开 `llm/stream` 边界采集。离线日志统计只覆盖其中已持久化的调用，缺失 usage 和不可见 query 不能作为零成本。运行方法与边界见 [评测说明](../eval-context-engineering.zh.md)。

本轮未运行付费模型 A/B，没有模型成功率、缓存净收益或账单改善结论；真实模型 Worker 重启恢复也仍需单独评测。确定性集成测试不能替代这些结果。若下一轮调整阈值、并发或结果契约，应冻结本轮版本，并按任务质量、全部可观察调用用量及未知比例分别比较。

验证记录：`npm run check:all` 通过（当时 113 个快速测试、212 个 integration 测试）；随后补充离线统计的消息配对回归，最终 `npm run check` 通过 115 个快速测试。新增模型套件独立 strict 类型检查通过；因没有 `DEEPSEEK_API_KEY`，两个模型测试文件共 5 个用例按门控跳过。真实 Prime prompt dump 已审阅，三个更新后的代码示例已执行检查；`lib/` 由构建同步。本轮没有安装或改动运行中的 TUI profile。
