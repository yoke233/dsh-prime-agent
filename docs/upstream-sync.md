...
- Prime child answers can inform the parent's active computation. DSH owns this invariant. DSH 0.1.2-rc.1 retired the child-scoped `tool-subagent-report` row and now advertises one `send_message({ agent_id, message })` on the official `tool-subagent-control` for parents and children alike. Every accepted message uses `Agent.steer()`, so a running target consumes it at its nearest step while an idle target starts a turn. The continuation manager also owns waking accounting and next-step FIFO ordering ahead of later settlement notices. This plugin composes that capability directly and no longer replaces the messaging row or maintains a private adapter.
...
- The Prime preset intentionally omits the shipped `code` preset's Plan Mode. Compaction keeps DSH's default policy, and the preset neither owns providers nor configures model capacity.
