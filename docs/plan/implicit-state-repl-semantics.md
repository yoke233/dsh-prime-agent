...
2026-08-14 复测:所有时间戳晚于 f8a2d62 的会话转录装的仍是旧 policy(DSH 长驻进程早于提交启动,policy 编译进插件 `lib/`),被动实测既不可靠(负载混杂、会话不独立、header `agentPreset` 字段失真)又太慢。据此调整交付原则:级 2(D8 keys 回显)与级 3(惯用式引导)随 v0.4 交接配方按设计判断一并交付,不再等待级 1 的证伪数据;需要对比证据时用主动评测离线产生(见 [context-capsule-handoff.md](context-capsule-handoff.md) 第 2 步)。测量脚本固化于 `scripts/measure-state-adoption.mjs`,分组以转录内 policy 文本为准。本方案(级 4)维持待命,仅由真实需求触发。
...
阶梯按维护成本升序。2026-08-14 起交付原则调整:级 1–3 均为零/低成本,按设计判断一并交付,不再以上一级的采用率数据为准入门槛(被动实测被证明不可靠且太慢,见下方复测记录);仅级 4 保留门槛,由真实需求触发而非指标触发:
...
级 2 与级 3 与 v0.4 交接配方合并交付:交接是同一惯用式的跨 Agent 作用域(材料先在 state 里才有得脱水,选交接 keys 需要 D8 回显可见)。提示词措辞需要对比证据时用主动评测(固定任务集 × policy 变体 × 自动评分)离线产生,不等真实使用积累;级 4 只由真实需求触发。合并节奏唯一定义在 [context-capsule-handoff.md](context-capsule-handoff.md) 的"启动条件与顺序"。
