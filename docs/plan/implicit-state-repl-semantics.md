...
1. **现在**:方案待命。policy 修复(f8a2d62)与 D8 keys 回显是更便宜的前置手段。
2. **观察**:收集 policy 修复后的真实会话,统计 `state` 引用率与样板重复率。
3. **判定**:若引导后模型仍不使用 state,实施本方案;若使用率起来,永久搁置。
4. 本方案与 IPython 之间不存在中间升级关系:若本方案实施后人机工学仍不达标,问题必然不在持久化语法,届时重新评估而非直接引入第二运行时。
