...
- Prime preset 有意不挂载 shipped `code` preset 的 Plan Mode。Compaction 仍由 DSH backend 拥有，但 Prime preset 在 `compaction-basic` 上多一条 `modelPolicies`：DSH 默认路由 `deepseek-official/deepseek-v4-flash` 的 `thresholdRatio` 降为 0.3（adapter 宣告 1M 窗口，出厂 0.8 在 Prime 的输出体量下形同不压缩；依据见 `docs/research/2026-code-mode-context-engineering.zh.md` §2、§5.2）；其他路由保持默认，preset 不接管 provider 或配置模型容量。同步上游 preset 时保留这条差异。
