...
function namespaceDeclaration(global, aliases) {
    if (aliases.length === 0)
        return undefined;
    const members = aliases.map(({ member, target }) => `  ${member}: (args: ToolArguments<${JSON.stringify(target)}>) => Promise<ToolOutputMap[${JSON.stringify(target)}]>;`);
    return `declare const ${global}: {\n${members.join('\n')}\n}`;
}
function projectToolDeclarations(declarations) {
