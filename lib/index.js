...
import { createLlmFunctions, LLM_BINDING_DEFAULTS, LLM_NAMESPACE_DOC, renderLlmMembers } from './llm/binding.js';
...
function replAgentPrompt(budgetBytes) {
...
const REPL_PATTERN_TOOLS = ['grep', 'read'];
...
function withoutCanonicalPwshPath(description) {
    if (!description.includes('`pwsh -Command`'))
        return description;
...
const TOOL_AGENT_DESCRIPTION_OVERRIDES = {
...
function namespaceDeclaration(global, aliases, extraMembers = [], doc = []) {
    if (aliases.length === 0 && extraMembers.length === 0)
        return undefined;
    const members = [
        ...aliases.map(({ member, target }) => `  ${member}: (args: ToolArgsMap[${JSON.stringify(target)}]) => Promise<ToolOutputMap[${JSON.stringify(target)}]>;`),
        ...extraMembers,
    ];
    const header = doc.length === 0 ? '' : `${doc.join('\n')}\n`;
    return `${header}declare const ${global}: {\n${members.join('\n')}\n}`;
}
function sdkText(ctx, agent, options) {
...
async function measureContext(ctx, agent, signal) {
...
function renderContextLine(value) {
...
export function apply(ctx, config) {
    ...
    const promptOptions = { visibleOutputBudgetBytes, llm: llmLimits };
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const leased = createReplBindings(ctx, exec, [continual.bindingFor(exec.agent)], createLlmFunctions(ctx, exec.agent, llmLimits, exec.signal));
            try {
...
export { LLM_BINDING_DEFAULTS } from './llm/binding.js';
