...
import { COMPLETION_EXPIRED_ERROR, COMPLETION_HISTORY_GLOBAL, LAST_RESULT_GLOBAL, MIN_OUTPUT_BYTES, OutputLedger, resolveCompletionHistoryLimits, resolveCompletionOpaqueLimits, resolveCompletionProjectionLimits, } from './protocol.js';
 ...
 */
const REALM_OWNED_GLOBALS = new Set([
...
function parseRunMetrics(raw) {
    ...
    const source = raw;
    const metrics = {};
    for (const key of ['captureBytes', 'captureNodes', 'evicted', 'expired', 'refused', 'historyEntries', 'historyBytes', 'historyOpaqueEntries', 'historyOpaqueBytes']) {
...
function parseCompletionPresentation(value) {
    ...
    if (handle !== undefined) {
        if (typeof handle !== 'number' || !Number.isSafeInteger(handle) || handle <= 0)
            return undefined;
        if (envelope.use !== `$out(${handle})` || envelope.retained === false || envelope.reason !== undefined)
            return undefined;
    ...
    if (envelope.use !== undefined || envelope.retained === true)
        return undefined;
    return {
...
export class PersistentRealm {
    ...
    recordRunMetrics(metrics = {}) {
        ...
        if (metrics.historyEntries !== undefined)
            this.counters.historyEntries = metrics.historyEntries;
        if (metrics.historyBytes !== undefined)
            this.counters.historyBytes = metrics.historyBytes;
        if (metrics.historyOpaqueEntries !== undefined)
            this.counters.historyOpaqueEntries = metrics.historyOpaqueEntries;
        if (metrics.historyOpaqueBytes !== undefined)
            this.counters.historyOpaqueBytes = metrics.historyOpaqueBytes;
    }
    /**
