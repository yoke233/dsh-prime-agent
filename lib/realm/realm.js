 ...
 */
function parseStateKeys(raw) {
    if (typeof raw !== 'object' || raw === null)
        return undefined;
    const { names, omitted } = raw;
    if (!Array.isArray(names) || !Number.isSafeInteger(omitted) || omitted < 0)
        return undefined;
    const rebuilt = [];
    ...
    return { names: rebuilt, omitted: omitted };
}
/**
    ...
    switch (message.type) {
        ...
        case 'done': {
            ...
            if (message.json === undefined)
                return { type: 'done', runId, nonce, ...census };
