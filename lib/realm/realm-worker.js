 ...
 */
function snapshotJson(value, projectRootRegExpPattern = false) {
    return snapshotValue(value, new CapturedSet(), newCaptureStats(UNLIMITED, UNLIMITED, UNLIMITED), undefined, projectRootRegExpPattern);
}
/**
 ...
 */
function snapshotValue(value, seen, stats, slot, projectRootRegExpPattern = false) {
    if (stats.aborted)
        return undefined;
    stats.nodes += 1;
    ...
    if (capturedArrayIsArray(source)) {
        ...
        for (let index = 0; index < keys.length; index++) {
            ...
            if (root && projectRootRegExpPattern && key === 'pattern'
                ...
                try {
                    projectedValue = capturedReflectApply(capturedRegExpSource, childValue, []);
                    const flags = capturedReflectApply(capturedRegExpFlags, childValue, []);
                    if (flags !== '') {
                    ...
                    if (error instanceof BindingArgumentProjectionError)
                        throw error;
...
function jsonEntryCount() {
    let count = 0;
    for (let index = 0; index < completionSlots.length; index++) {
        if (!completionSlots[index].opaque)
            count += 1;
    }
    return count;
}
/** Opaque slots currently held, whatever the shared store's total length. */
function opaqueEntryCount() {
    let count = 0;
    for (let index = 0; index < completionSlots.length; index++) {
        if (completionSlots[index].opaque)
            count += 1;
...
function evictOldestOpaqueCompletion() {
    for (let index = 0; index < completionSlots.length; index++) {
        const slot = completionSlots[index];
        if (!slot.opaque)
            continue;
...
function opaqueHistoryFits(bytes, nodes) {
...
function retainCompletion(value, bytes, stats) {
    ...
    if (stats.overNodeLimit || !completionFitsAlone(bytes, stats.nodes))
        return undefined;
    while (jsonEntryCount() > 0 && !completionHistoryFits(bytes, stats.nodes))
...
function retainOpaqueCompletion(value, stats) {
    ...
    if (bytes > opaqueLimits.maxCompletionOpaqueEstimatedBytes
        || nodes > opaqueLimits.maxCompletionOpaqueNodes)
        return undefined;
    while (opaqueEntryCount() > 0 && !opaqueHistoryFits(bytes, nodes))
        evictOldestOpaqueCompletion();
    ...
    if (!opaqueHistoryFits(bytes, nodes))
        return undefined;
    const slot = capturedObjectCreate(null);
    slot.id = run.completionId;
    slot.value = value;
...
const completionList = () => {
    ...
    for (let index = 0; index < completionSlots.length; index++) {
        const slot = completionSlots[index];
        const row = capturedObjectCreate(null);
        row.id = slot.id;
        row.type = slot.type;
 ...
 */
function projectCompletion(value, id, bytes, projection, remaining, maxOutputBytes) {
 ...
 */
function projectOpaqueCompletion(value, id, remaining, maxOutputBytes) {
    const budget = projectionLimits.maxCompletionProjectionBytes < remaining
        ? projectionLimits.maxCompletionProjectionBytes
        : remaining;
    const type = completionType(value);
    ...
    if (id === undefined)
        rich.reason = 'opaque history budget exceeded';
    rich.truncated = true;
    const richJson = capturedJsonStringify(rich);
    if (capturedBufferByteLength(richJson, 'utf8') <= budget)
        return { json: richJson };
