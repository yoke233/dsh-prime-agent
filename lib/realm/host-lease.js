...
function leaseFailure(error) {
    ...
    if (message === HELD || message === CORRUPTED)
 ...
 */
export async function acquireHostLease(directory) {
    ...
    return async () => {
        try {
            await withFileLock(filename, async () => {
                ...
                if (parseLease(current)?.nonce !== claim.nonce)
                    return;
...
//# sourceMappingURL=host-lease.js.map