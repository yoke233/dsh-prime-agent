...
function handshakeFailure(error) {
    const message = error instanceof Error ? error.message : '';
    if (message.startsWith('prime-realm-identity: '))
...
export function registerRealmIdentity(ctx, options) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            if (args.protocol !== 1)
                throw new Error('prime-realm-identity: unsupported handshake protocol');
            ...
            return { protocol: 1, token: issued.token, proof: issued.proof };
        },
    }));
}
