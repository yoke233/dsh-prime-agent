...
import { MIN_OUTPUT_BYTES, OutputLedger, resolveCompletionHistoryLimits, resolveCompletionOpaqueLimits, resolveCompletionProjectionLimits, } from './protocol.js';
...
function namespaceNotice(fresh, lost) {
    if (lost)
        return '[prime-realm] live namespace restarted; previous bindings and retained results were lost';
    if (fresh)
...
export class PrimeRealmRuntime extends Service {
     ...
     */
    retiredMetrics = emptyRealmMetrics();
    ...
    get metrics() {
        ...
        for (const realm of this.pool.values())
            addRealmMetrics(total, realm.metrics);
        return total;
    }
    /**
     ...
     */
    async run(realmId, request) {
        if (this.disposed)
            throw new Error('dsh-prime-agent: prime realm runtime run() after disposal');
