 ...
 */
function generationNotice(generation, fresh, lost) {
    if (!fresh)
        return `[prime-realm] generation ${generation} retained`;
    return lost
...
function stateKeysNotice(keys) {
    ...
    for (const name of keys.names) {
        ...
        if (shown.length >= MAX_STATE_KEYS || bytes + cost > MAX_STATE_LINE_BYTES)
            break;
        bytes += cost;
        shown.push(safe);
    }
...
export class PrimeCodeRuntime extends CodeRuntime {
    ...
    async run(request) {
        ...
        if (handshake === undefined)
            return this.exception('realm handshake returned a malformed response');
        let verification;
        try {
        ...
        if (!verification.ok)
            return this.exception(`realm handshake rejected: ${verification.reason}`);
        if (signal?.aborted)
            return this.aborted(signal);
    ...
    async execute(realmId, request) {
        ...
        const notice = dispatched ?? generationNotice(entry.generationBase + entry.realm.generation, pendingLoss || !entry.everDispatched, pendingLoss);
    ...
    sweepIdle() {
        ...
        for (const [realmId, entry] of [...this.pool]) {
            if (!entry.realm.idle || entry.realm.lastUsedAt > deadline)
                continue;
