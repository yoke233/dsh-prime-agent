...
async function dispatchLogContent(ctx, dispatch) {
    try {
        return await ctx.waterfall('tools/ptc-dispatch-log', dispatch, () => Promise.resolve(dispatch.content));
    }
    catch {
 ...
 */
export function createReplBindings(ctx, exec, extraBindings = [], agentFunctions = {}) {
    ...
    const namespace = (global, aliases, extras = {}) => {
        ...
        for (const [member, fn] of Object.entries(extras)) {
            if (Object.hasOwn(aliases, member))
                throw new Error(`dsh-prime-agent: ${global}.${member} is a delegation alias and cannot be replaced`);
