...
function ownerFor(scope, agent, allowGlobal) {
    if (scope === 'global') {
        if (!allowGlobal)
            throw new Error('prime-context: global workspace access is disabled by deployment policy');
        return 'global';
    }
    if (agent === undefined)
        throw new Error('prime-context: local workspace operations require an owning agent session');
    return String(agent.id);
}
function assertOnly(args, operation, allowed) {
...
function orchestrationPolicy(ctx, agent, toolName, requireTools) {
...
export function registerRlm(ctx, config) {
    ...
    ctx.systemPrompt.context({
        ...
        text: (assembly) => {
            ...
            if (config.allowGlobal)
                catalogs.unshift(renderContextCatalog(store.readManifestSync('global', 'global'), config.limits));
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (args.operation === 'put') {
                ...
                const { manifest, entry } = await store.put(scope, owner, args.expected_revision, args.key, args.kind, args.summary, args.value);
                return { operation: 'put', scope, revision: manifest.revision, result: { entry } };
            }
            if (args.operation === 'get') {
                assertOnly(args, 'get', ['key', 'offset', 'limit', 'pointer']);
                if (args.key === undefined)
                    throw new Error('prime-context: get requires key');
                const read = await store.get(scope, owner, args.key, {
                    ...(args.offset === undefined ? {} : { offset: args.offset }),
                    ...(args.limit === undefined ? {} : { limit: args.limit }),
                    ...(args.pointer === undefined ? {} : { pointer: args.pointer }),
                });
                return { operation: 'get', scope, revision: read.manifestRevision, result: read };
            }
            if (args.operation === 'search') {
                assertOnly(args, 'search', ['query', 'key', 'case_sensitive', 'limit']);
                if (args.query === undefined)
                    throw new Error('prime-context: search requires query');
                ...
                return { operation: 'search', scope, revision: result.manifestRevision, result: result };
            }
            ...
            return { operation: 'delete', scope, revision: manifest.revision, result: { deleted } };
        },
    }));
    return store;
}
