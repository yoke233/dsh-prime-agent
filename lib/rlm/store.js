...
function isHighSurrogate(value, index) {
    const unit = value.charCodeAt(index);
    return unit >= 0xD800 && unit <= 0xDBFF;
}
/** Slice on UTF-16 boundaries without emitting an isolated surrogate. */
function boundedUnicodeSlice(value, start, limit) {
    ...
    if (end < value.length && isLowSurrogate(value, end) && isHighSurrogate(value, end - 1))
        end--;
...
function boundedUnicodePrefix(value, limit) {
    let end = Math.min(value.length, limit);
    if (end < value.length && isLowSurrogate(value, end) && isHighSurrogate(value, end - 1))
        end--;
    return value.slice(0, end);
}
function unicodeWindow(value, start, end) {
    let safeStart = start;
    let safeEnd = Math.min(value.length, end);
    if (safeStart > 0 && isLowSurrogate(value, safeStart) && isHighSurrogate(value, safeStart - 1))
        safeStart--;
    if (safeEnd < value.length && isLowSurrogate(value, safeEnd) && isHighSurrogate(value, safeEnd - 1))
        safeEnd--;
