...
async function loadCodeMode() {
    try {
        ...
        throw new Error('dsh-prime-agent: the packaged Code Mode dependencies could not be loaded '
            + '(@deepseek-ai/dsh-code-runtime and @deepseek-ai/dsh-code-runtime-worker-thread); '
            + 'reinstall dsh-prime-agent and verify that the host uses a compatible DSH 0.1.x release', { cause: error });
    }
}
/**
