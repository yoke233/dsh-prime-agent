...
function readPage(value, path, expectedOffset) {
    ...
    if (!Array.isArray(lines) || !Number.isInteger(totalLines) || totalLines < 0 || offset !== expectedOffset) {
        throw new PatchError(PATCH_ERROR_CODES.snapshotReadFailed, `read returned an invalid snapshot for ${path}`, { path });
    }
    const parsed = [];
    ...
    for (const line of lines) {
        if (typeof line !== 'object' || line === null || Array.isArray(line)
            || line.number !== nextNumber || line.number > totalLines || typeof line.text !== 'string') {
    ...
    return { lines: parsed, totalLines: totalLines };
}
function forwardNestedResult(exec, result) {
...
export async function executePatch(ctx, args, exec) {
    ...
    const dispatch = async (name, argumentsValue) => {
    ...
    for (const file of parsed.files) {
        ...
        while (true) {
            ...
            if (result.isError) {
                ...
                throw new PatchError(PATCH_ERROR_CODES.snapshotReadFailed, `could not preflight ${file.path}: ${result.error.message}`, { path: file.path });
            }
