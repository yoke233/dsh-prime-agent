...
const ENTRY_KINDS = new Set(['prompt', 'memory', 'skill', 'subagent']);
function emptyState(scope, owner) {
...
function isEntry(value) {
    if (!isRecord(value) || !exactKeys(value, ['id', 'kind', 'title', 'content', 'createdAt', 'updatedAt'], ['reference']))
        return false;
    if (typeof value.id !== 'string' || typeof value.kind !== 'string' || !ENTRY_KINDS.has(value.kind))
        return false;
    ...
    if (value.reference === undefined)
        return value.kind === 'prompt' || value.kind === 'memory';
    if ((value.kind !== 'skill' && value.kind !== 'subagent') || !isRecord(value.reference))
        return false;
...
function isTransaction(value) {
    if (!isRecord(value) || !exactKeys(value, ['id', 'type', 'trigger', 'evidence', 'expectedOutcome', 'createdAt', 'changes'], ['rollbackOf']))
        return false;
...
function parseState(text, scope, owner, limits) {
    let value;
    try {
    ...
    if (value.schemaVersion !== 1 || value.scope !== scope || value.owner !== owner || !Number.isSafeInteger(value.revision) || value.revision < 0) {
    ...
    for (const entry of value.entries) {
        ...
        if (ids.has(key))
            throw new Error(`prime-agent: duplicate entry ${key} in ${scope}/${owner}`);
        ids.add(key);
...
function normalizeText(value, label, maxChars) {
    ...
    if (normalized.length === 0)
        throw new Error(`prime-agent: ${label} must not be empty`);
    if (normalized.length > maxChars)
        throw new Error(`prime-agent: ${label} exceeds ${maxChars} characters`);
    return normalized;
}
function boundedText(value, maxChars) {
    return value.length <= maxChars ? value : value.slice(0, maxChars);
}
function assertEntryWithinLimits(entry, label, limits) {
    if (entry.id.trim().length === 0 || entry.id.length > limits.maxEntryIdChars) {
        throw new Error(`prime-agent: ${label} has an invalid entry id`);
    }
    if (entry.title.trim().length === 0 || entry.title.length > limits.maxEntryTitleChars) {
        throw new Error(`prime-agent: ${label} has an invalid title`);
    }
    if (entry.content.trim().length === 0 || entry.content.length > limits.maxEntryContentChars) {
        throw new Error(`prime-agent: ${label} has invalid or oversized content`);
    }
    if (entry.reference !== undefined
        && (entry.reference.tool.trim().length === 0 || entry.reference.tool.length > limits.maxReferenceToolChars)) {
        throw new Error(`prime-agent: ${label} has an invalid callable reference`);
    }
}
function assertTransactionWithinLimits(transaction, limits) {
...
function renderState(state, limits) {
...
export class HarnessStore {
    ...
    readSync(scope, owner) {
        const filename = this.path(scope, owner);
        if (!existsSync(filename))
            return emptyState(scope, owner);
    ...
    async apply(scope, owner, expectedRevision, trigger, evidence, expectedOutcome, edits) {
        ...
        return this.mutate(scope, owner, expectedRevision, (current) => {
            ...
            for (const edit of edits) {
                if (!ENTRY_KINDS.has(edit.kind))
                    throw new Error(`prime-agent: invalid entry kind ${String(edit.kind)}`);
                ...
                if (touched.has(key))
                    throw new Error(`prime-agent: transaction edits ${key} more than once`);
                touched.add(key);
                const index = entries.findIndex(entry => entry.kind === edit.kind && entry.id === id);
                const before = index < 0 ? null : cloneEntry(entries[index]);
                if (edit.action === 'create' && before !== null)
                    throw new Error(`prime-agent: create conflicts with existing ${key}`);
                if ((edit.action === 'update' || edit.action === 'delete') && before === null)
                    throw new Error(`prime-agent: ${edit.action} cannot find ${key}`);
                let after = null;
                if (edit.action !== 'delete') {
                    ...
                    if (callable && edit.reference === undefined)
                        throw new Error(`prime-agent: ${key} requires a callable reference`);
                    if (!callable && edit.reference !== undefined)
                        throw new Error(`prime-agent: ${key} must not declare a callable reference`);
                    let reference;
                    if (edit.reference !== undefined) {
                        ...
                        if (!isJsonValue(edit.reference.arguments))
                            throw new Error(`prime-agent: ${key} reference.arguments is not lossless JSON`);
    ...
    async rollback(scope, owner, expectedRevision, transactionId) {
        ...
        return this.mutate(scope, owner, expectedRevision, (current) => {
            ...
            if (target === undefined)
                throw new Error(`prime-agent: retained transaction ${targetId} was not found`);
            const entries = current.entries.map(cloneEntry);
...
export function renderHarnessState(state, limits) {
    if (state.entries.length === 0)
        return `${state.scope} harness revision ${state.revision}: empty`;
    ...
    const lines = [`${state.scope} harness revision ${state.revision}:`];
    let used = lines[0]?.length ?? 0;
    for (const entry of newest) {
        const reference = entry.reference === undefined
            ? ''
            : `\n  callable: tools.${entry.reference.tool}(${JSON.stringify(entry.reference.arguments)})`;
        const block = `- [${entry.kind}:${entry.id}] ${entry.title}\n  ${entry.content}${reference}`;
        if (used + block.length + 1 > limits.maxPromptCharsPerScope) {
            lines.push('- [remaining entries omitted by prompt budget]');
            break;
        }
        lines.push(block);
        used += block.length + 1;
    }
    if (newest.length < state.entries.length)
        lines.push(`- [${state.entries.length - newest.length} older entries omitted]`);
    return lines.join('\n');
}
