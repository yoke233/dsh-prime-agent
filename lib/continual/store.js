...
function snapshotMatchesChange(entry, change) {
...
function isChange(value) {
    return isRecord(value)
        ...
        && snapshotMatchesChange(value.before, value)
        && snapshotMatchesChange(value.after, value);
}
...
function assertPromptRecordText(value, label) {
...
function assertEntryWithinLimits(entry, label, limits) {
    ...
    if (entry.reference !== undefined)
        assertSingleLine(entry.reference.tool, `${label} callable reference`);
}
...
export class HarnessStore {
            ...
            for (const edit of edits) {
                ...
                if (edit.action !== 'delete') {
                    ...
                    if (!isWellFormedUnicode(content))
                        throw new Error(`prime-agent: ${key} content must contain well-formed Unicode`);
    ...
    async rollback(scope, owner, expectedRevision, transactionId, trigger) {
...
export function renderHarnessState(state, limits) {
    ...
    if (omitted > 0 && used + omittedLine.length + 1 <= limits.maxPromptCharsPerScope)
