...
export const LLM_BINDING_DEFAULTS = {
...
function record(value, member) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
        throw new Error(`${member}() takes one object argument`);
    }
    return value;
}
function optionalText(value, member, field, limit) {
    if (value === undefined)
        return undefined;
    if (typeof value !== 'string')
        throw new Error(`${member}() ${field} must be a string`);
    if (value.length > limit)
        throw new Error(`${member}() ${field} is ${value.length} characters; the limit is ${limit}. Slice or split it first`);
    return value;
}
function requiredText(value, member, field, limit) {
    const text = optionalText(value, member, field, limit);
    if (text === undefined || text.trim().length === 0)
        throw new Error(`${member}() ${field} must be a non-empty string`);
    return text;
}
function maxTokensOf(value, member, limit) {
    if (value === undefined)
        return limit;
...
function rejectUnknownKeys(args, member, allowed) {
    for (const key of Object.keys(args)) {
        if (!allowed.includes(key))
            throw new Error(`${member}() received unknown argument ${JSON.stringify(key)}`);
    }
}
function parseQuery(value, limits) {
...
function parseBatch(value, limits) {
    ...
    if (!Array.isArray(args.prompts) || args.prompts.length === 0)
        throw new Error(`${QUERY_MANY}() prompts must be a non-empty array`);
...
async function generate(ctx, agent, request, signal) {
    signal.throwIfAborted();
    const llm = ctx.get('llm');
    if (llm === undefined)
        throw new Error('the LLM service is unavailable in this session');
    ...
    if (finish.kind === 'error' || finish.kind === 'aborted')
        throw new Error(`model call failed: ${finish.failure.message}`);
    if (finish.kind !== 'stop' && finish.kind !== 'max-tokens')
        throw new Error(`model call stopped unexpectedly: ${finish.kind}`);
    return { text: textOf(assembler.blocks()), truncated: finish.kind === 'max-tokens' };
}
async function mapBounded(items, concurrency, parentSignal, operation) {
    ...
    const worker = async () => {
        while (!batchAbort.signal.aborted && next < items.length) {
            const index = next++;
            try {
                results[index] = await operation(items[index], index, batchAbort.signal);
            }
            catch (error) {
...
export function createLlmFunctions(ctx, agent, limits, signal) {
    return {
        [LLM_QUERY_MEMBER]: async (value) => await generate(ctx, agent, parseQuery(value, limits), signal),
...
export const LLM_NAMESPACE_DOC = [
    '/**',
...
export function renderLlmMembers(limits) {
    return [
