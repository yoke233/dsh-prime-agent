...
- `dsh-prime-agent/context-manager` 的压力阈值默认 0.8，Prime preset 为 `deepseek-official/deepseek-v4-flash` 保留 0.3；普通压力处理保留约 16000 tokens 的最近尾部，并按工具配对边界调整。模型可先保存笔记，再调用 `tools.new_context({})` 请求下一 step 切换；主动切换与溢出恢复使用 DSH 的最小安全尾部策略。`/compact` 也使用同一目录替换方式。preset 不注册 provider、不修改模型窗口容量。
...
模型指导以决策证据为准：归约保留来源、关键例外和未决项；单次调用足以回答时可直接使用。重要进展和用户纠正后更新任务笔记，主动切窗前保持笔记当前；自动切窗后的空笔记或旧笔记需结合历史原文核验。历史可回取不等于模型一定能自行恢复，任务级模型评测与成本统计见 [上下文工程评测](docs/eval-context-engineering.zh.md)。
- Prime preset 为模型可见的工具结果配置 12KB best-effort spill 阈值，固定提示把同一数字作为每 cell 的事前显示预算告知模型（插件 `visibleOutputBudgetBytes`，默认 12000，须与 preset 的 `maxInlineBytes` 保持一致）；每个 `repl` 结果末尾追加一行 `Context: <已用> / <窗口> tokens`（host `tokenMeter` 对当前 Session 的估算与路由模型宣告的 contextWindow，在 cell 开始前测量，任一不可用时整行省略，对应 canonical value 的可选 `contextTokens`/`contextWindow`）。`repl` 的外层 canonical value 仍是可程序化读取的 lossless JSON（`logs`、可选 `result`、可信 presentation metadata 与可选上下文用量），但模型只看到无类型外壳的 notebook 文本：logs 和字符串原样显示，结构化值只 pretty-print 一次，空结果返回空文本；renderer 不添加 `[repl result: ...]`、`[repl logs]` 或 Markdown fence。展示缩短不改变已赋给 REPL 变量的 canonical value；spill notice 出现时，模型先从变量继续结构化处理，确需遗漏的格式化文本才 read/grep locator，并在 Windows 字符串中把 locator 的反斜杠改为正斜杠。TypeScript 解析错误只提取 Node parser stack 中的 cell 行列，不回显源码片段；普通程序异常只保留可行动的异常消息，不输出用户栈；成功值仍保留 canonical completion。
...
会话轨迹保存在 `$DSH_HOME/sessions`；Prime Realm 身份、任务笔记与学习状态分开保存在 `$DSH_HOME/prime-agent`。

## 编排工作流

控制面 policy 把 TypeScript 定位为编排语言：循环、条件、解析、过滤、聚合与状态都在 cell 内完成，工具调用是普通 `await` 表达式，其类型化结果在同一 cell 内绑定、切片、组合；读取、搜索和命令结果绑定到命名变量，仅在输入与来源仍有效时复用；编辑、外部写入、child 修改或用户纠正后刷新受影响证据，并重新判断派生索引与计划。固定提示补上 code mode 的经济学：只有 cell 的 completion 与 `console.log` 进入对话，其余结果留在 REPL；每 cell 可见输出预算约 12 KB，超出即落盘换 preview，因此先在代码里 filter/slice/count/aggregate 再显示；额外往返比额外输出更贵，先减 cell 数，再在不增加往返的前提下收紧显示；「一个 cell 只有一个 `await tools.x()` 且原始结果直接成为 completion」是把 REPL 当语法糖的红旗，反向条款是一次 grep 或一次范围 read 能定位就直接读。不确定的文件路径从已知父目录 `glob`，不确定的目录路径通过 `pwsh` 检查父目录；只有任一失败会让全部成功结果都失去用途时才用 `Promise.all`，相互独立的读取、搜索和探测即使希望拿到全部答案也改用 `Promise.allSettled` 或逐项捕获 `ToolCallError`，保留成功结果并只重试失败项；副作用型 mutation 顺序执行。超过 64 KiB 的完成值由 runtime 生成有界 preview；原值通过准入时仍在 `$_`，应在运行下一个产生 completion 的 cell 前赋给命名变量，并从该变量继续切片、过滤或计数，不再整体显示。compaction 生成的 checkpoint 不影响 Realm：变量与函数仍然活着，来源仍有效时可直接继续；只有 `live namespace restarted` 通知才意味着 namespace 丢失，此时从文件重建并核对外部状态后再恢复 mutation。

慢任务使用非阻塞控制循环：交给 managed Job 或 continuable child，保存 id/输出位置后继续独立工作，或结束当前 turn 等待通知；不使用 sleep 轮询或长阻塞 `await` 占住交互。多回合或多 child 工作由直接面向用户的 root 在有意义里程碑简洁汇报结果、阻塞和下一步。委派判据：并行的重上下文调研或独立实现交给 agent，单次已知的查找、编辑或命令直接在 cell 内做；会淹没对话的探索（大范围搜索、长日志、多文件）交给 agent 并只要结论、计数和路径，不要转录。`subagent` 的 JSDoc 要求 prompt 含目标、报告格式、相关路径与边界，child 以几百字回报，大材料写文件。
