...
The plugin reuses DSH's Agent Loop, TypeScript Code Runtime, Subagent, Jobs, Goal, Workflow, Session, and cancellation semantics. It does not embed IPython or create a second worker lifecycle.
...
The important Prime Agent property is a programmable control plane over addressable context and asynchronous agents, not Python syntax itself. DSH already supplies a TypeScript Code Runtime with scoped tools, logging, cancellation, Subagent, and Jobs integration. Embedding a persistent IPython kernel would introduce another RPC, process, recovery, isolation, and ownership boundary.

Version 0.2 therefore persists data in `prime_context` while each `run_code` invocation remains ephemeral. A Python runtime should only be considered later if Python object identity and kernel compatibility become explicit product requirements.
